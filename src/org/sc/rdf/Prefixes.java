...
public class Prefixes {
	...
	public String prettyPrint(Value obj) { 
		String objString = obj.toString();
		if(obj instanceof Literal) { 
			Literal lit = (Literal)obj;
			String lbl = lit.getLabel();
			URI datatype = lit.getDatatype();
			if(lbl != null && datatype != null) { 
				objString = String.format("\"%s\"^^%s", 
						lit.getLabel(), 
						contract(lit.getDatatype().toString()));
			}
		} else if (obj instanceof Resource) { 
			if(obj instanceof URI) { 
				objString = contract(objString);
			}
		}
		return objString;
	}
	
	public String prettyPrint(Statement stmt) { 
		StringBuilder sb = new StringBuilder();
		Resource subj = stmt.getSubject();
		URI pred = stmt.getPredicate();
		Value obj = stmt.getObject();
		
		String subjString = prettyPrint(subj);
		String predString = prettyPrint(pred);
		String objString = prettyPrint(obj);
		sb.append(String.format("(%s %s %s)", 
				subjString, predString, objString));
		
		return sb.toString();
	}
	
	public URI asURI(String prefix, String extension) { 
		return new URIImpl(String.format("%s%s", prefixMap.get(prefix), extension));
	}
